...
async function downloadWorkloadArtifacts(options) {
  ...
  for (let [workload, files] of workloadFiles) {
    if (!files.pull || !files.metrics) {
      import_core.debug(`Skipping artifact '${workload}': not a workload artifact (missing -pull.txt or -metrics.jsonl). This is expected for user-uploaded artifacts like logs.`);
      continue;
    }
    try {
...
async function main() {
  try {
    ...
    fs3.mkdirSync(cwd, { recursive: !0 }), import_core6.info(`Working directory: ${cwd}`), import_core6.info("\uD83D\uDD0D Fetching workflow run information...");
    let octokit = import_github3.getOctokit(token), { data: workflowRun } = await octokit.rest.actions.getWorkflowRun({
      owner: import_github3.context.repo.owner,
      repo: import_github3.context.repo.repo,
      run_id: runId
    }), headSha = workflowRun.head_sha;
    import_core6.info(`Workflow run SHA: ${headSha}`), import_core6.info("\uD83D\uDCE6 Downloading artifacts from current run...");
